pipeline {
    ...
    environment {
		...
		AWS_ACCESS_KEY_ID = credentials('gamification-api-prod-aws-access-key-id')
		AWS_SECRET_ACCESS_KEY = credentials('gamification-api-prod-aws-secrect-access-key')
		S3_REGION = 'eu-west-1'
        ...
        stage('Test') {
            steps {
				sh '''
					...
					docker-compose --no-ansi run --rm --no-deps -u $(id -u jenkins):$(id -g jenkins) app "python3 --version"
				'''
            }
        }
		stage('Build') {
